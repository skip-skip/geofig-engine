...
def validate_figure_spec(spec: FigureSpec) -> None:
    ...
    # Validate children: depth-1 only, each must be a FigureSpec
    if spec.children:
        if not isinstance(spec.children, tuple):
            raise TypeError(f"children must be a tuple of FigureSpec, got {type(spec.children).__name__}")
        for i, child in enumerate(spec.children):
            if not isinstance(child, FigureSpec):
                raise TypeError(
                    f"children[{i}] must be a FigureSpec, got {type(child).__name__}"
                )
            if child.children:
                raise ValueError(
                    f"children[{i}] has its own children; depth > 1 not supported"
                )

    if spec.transform is not None and not isinstance(spec.transform, LinkTransform):
        raise TypeError(
            f"transform must be a LinkTransform, "
